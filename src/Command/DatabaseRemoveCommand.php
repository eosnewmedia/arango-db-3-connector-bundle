 ...
 */
class DatabaseRemoveCommand extends AbstractDatabaseCommand
{
    /**
    ...
    {
        parent::__construct('database:remove', $databaseClient);
    ...
    {
        try {
            $this->getDatabaseClient()->database()->removeDatabase(
                $input->getArgument('username'),
                $input->getArgument('password')
            );

            (new SymfonyStyle($input, $output))->success('Database removed!');
        } catch (\Throwable $e) {
